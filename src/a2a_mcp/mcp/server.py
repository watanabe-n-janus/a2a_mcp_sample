...
def serve(host, port, transport):  # noqa: PLR0915
    ...
    if df is None:
        logger.error('Failed to build agent card embeddings. Exiting.')
        raise ValueError('Failed to load agent cards. Please check logs for details.')
    ...
    def find_agent(query: str) -> str:
        ...
        Returns:
            The json string representing the agent card deemed most relevant
            to the input query based on embedding similarity.
        """
        if df is None or df.empty:
            logger.error('Agent card embeddings are not available')
            return json.dumps({'error': 'Agent cards not loaded'})
    ...
    @mcp.tool()
    def search_attractions(city: str, category: str = None, min_rating: float = None) -> str:
        """観光地を検索します。都市名、カテゴリ、最低評価で絞り込みできます。
        
        Args:
            city: 検索する都市名（例: Paris, Tokyo, London）
            category: カテゴリでフィルタ（Landmark, Museum, Park, Historic, Religious, Shopping, Entertainment）
            min_rating: 最低評価（0.0-5.0）
        
        Returns:
            JSON形式の観光地リスト
        """
        logger.info(f'Searching attractions in {city}, category={category}, min_rating={min_rating}')
        ...
        try:
            ...
            logger.error(f'Exception searching attractions: {e}')
